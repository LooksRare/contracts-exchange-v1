 ...
 * @notice It is a staking pool for Uniswap V2 LP tokens (stake Uniswap V2 LP tokens -> get EARTH).
 */
contract StakingPoolForUniswapV2Tokens is Ownable, Pausable, ReentrancyGuard {
    ...
    // EARTH token (token distributed)
    IERC20 public immutable earthRareToken;

    // The staked token (i.e., Uniswap V2 WETH/EARTH LP token)
    ...
    // Tokens distributed per block (in EarthRareToken)
     ...
     * @param _earthRareToken reward token address
     * @param _rewardPerBlock reward per block (in EARTH)
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        if (userInfo[msg.sender].amount > 0) {
            ...
            if (pendingRewards > 0) {
                earthRareToken.safeTransfer(msg.sender, pendingRewards);
            }
        }
     ...
     * @param amount amount to withdraw (in EarthRareToken)
