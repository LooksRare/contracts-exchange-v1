 ...
 * WETH along with the auto-compounding of EARTH.
 */
contract FeeSharingSystem is ReentrancyGuard, Ownable {
    ...
    /**
     * @notice Constructor
     * @param _earthRareToken address of the token staked (EARTH)
     ...
     * @dev There is a limit of 1 EARTH per deposit to prevent potential manipulation of current shares
     */
    function deposit(uint256 amount, bool claimRewardToken) external nonReentrant {
        require(amount >= PRECISION_FACTOR, "Deposit: Amount must be >= 1 EARTH");
        ...
        // Verify EARTH token allowance and adjust if necessary
        _checkAndAdjustEARTHTokenAllowanceIfRequired(amount, address(tokenDistributor));
    ...
    /**
     * @notice Calculate value of EARTH for a user given a number of shares owned
     * @param user address of the user
     */
     ...
     */
    function _checkAndAdjustEARTHTokenAllowanceIfRequired(uint256 _amount, address _to) internal {
        if (earthRareToken.allowance(address(this), _to) < _amount) {
            earthRareToken.approve(_to, type(uint256).max);
        }
    }
    ...
    function _withdraw(uint256 shares, bool claimRewardToken) internal {
        ...
        // Retrieve total amount staked and calculated current amount (in EARTH)
        (uint256 totalAmountStaked, ) = tokenDistributor.userInfo(address(this));
        ...
        // Transfer EARTH tokens to sender
        earthRareToken.safeTransfer(msg.sender, currentAmount);
