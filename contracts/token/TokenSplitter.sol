...
/**
 * @title TokenSplitter
 * @notice It splits EARTH to team/treasury/trading volume reward accounts based on shares.
 */
contract TokenSplitter is Ownable, ReentrancyGuard {
    ...
    // Total EARTH tokens distributed across all accounts
    ...
    /**
     * @notice Release EARTH tokens to the account
    ...
    function releaseTokens(address account) external nonReentrant {
        ...
        uint256 totalTokensReceived = earthRareToken.balanceOf(address(this)) + totalTokensDistributed;
        ...
        earthRareToken.safeTransfer(account, pendingRewards);
    ...
    /**
     * @notice Retrieve amount of EARTH tokens that can be transferred
     * @param account address of the account
     */
