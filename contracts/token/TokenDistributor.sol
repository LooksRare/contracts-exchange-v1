 ...
 * @notice It handles the distribution of EARTH token.
 * It auto-adjusts block rewards over a set number of periods.
 */
contract TokenDistributor is ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeERC20 for IEarthRareToken;
    ...
    IEarthRareToken public immutable earthRareToken;
    ...
    /**
     * @notice Constructor
     * @param _earthRareToken EARTH token address
    ...
    constructor(
        ...
        uint256 nonCirculatingSupply = IEarthRareToken(_earthRareToken).SUPPLY_CAP() -
            IEarthRareToken(_earthRareToken).totalSupply();
        ...
        // 2. Store values
        earthRareToken = IEarthRareToken(_earthRareToken);
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        // Transfer EARTH tokens to this contract
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        // Transfer EARTH tokens to the sender
        earthRareToken.safeTransfer(msg.sender, amount);
    ...
    function _updatePool() internal {
        ...
        if (tokenRewardForStaking > 0) {
            ...
            bool mintStatus = earthRareToken.mint(address(this), tokenRewardForStaking);
            if (mintStatus) {
                accTokenPerShare = accTokenPerShare + ((tokenRewardForStaking * PRECISION_FACTOR) / totalAmountStaked);
            }

            earthRareToken.mint(tokenSplitter, tokenRewardForOthers);
        }
