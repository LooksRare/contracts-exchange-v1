 ...
 * @notice It distributes EARTH tokens with parallel rolling Merkle airdrops.
...
contract MultiRewardsDistributor is Pausable, ReentrancyGuard, Ownable {
    ...
    // Time buffer for the admin to withdraw EARTH tokens if the contract becomes paused
    uint256 public constant BUFFER_ADMIN_WITHDRAW = 3 days;

    // Standard safe guard amount (set at 1 EARTH)
    ...
    // Total amount claimed by user (in EARTH)
    ...
    /**
     * @notice Constructor
     * @param _earthRareToken address of the EarthRare token
     */
    constructor(address _earthRareToken) {
    ...
    /**
     * @notice Transfer EARTH tokens back to owner
